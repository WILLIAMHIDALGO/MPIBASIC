...
#define TAGFILAS 3 // marca que denota que ya no hay mas mensajes por enviar
...
int distributereceive(int f, int c, int *m, int *x, int size) {
	...
	*/
	...
	int xx[c];
        ...
        int t,resultado;
         MPI_Status Stat;
        for (i = 1; i < size; i++){//Hacer esto para cada proceso,for que recorre el numero de procesos empezando del proceso 1


        int k, j;
        //For que recorre la matriz y le manda una fila por fila a los procesos
        for (k = 0; k< f; k++) {
                for (j = 0; j < c; j++){
        ...
        MPI_Send(x,c,MPI_INT,i,TAGFILAS,MPI_COMM_WORLD);
        ...
        //MPI_Recv(resultado, 1, MPI_INT, 1, TAGTAREA, MPI_COMM_WORLD, &Stat);  
...
int receive(int rank) {
	...
	*/
	MPI_Status Stat;
        ...
        MPI_Recv(t, 5, MPI_INT, 0, TAGFILAS, MPI_COMM_WORLD, &Stat);

        for (i=0; i < 5; i++) {

        }

        //MPI_Send(resultado,1,MPI_INT,0,TAGTAREA,MPI_COMM_WORLD);
